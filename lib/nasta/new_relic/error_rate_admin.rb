...
module NewRelic
  class ErrorRateAdmin
    def newrelic_request
      from = Time.now - 60*5
      to = Time.now
      path = "https://api.newrelic.com/v2/applications/#{ENV['NR_APP_ID_ADMIN']}/metrics/data.json?names[]=WebTransaction&names[]=Errors%2Fall&from=#{from.iso8601(0)}&to=#{to.iso8601(0)}"
      puts path
      uri = URI(path)
